"""Text store for wid -> assembled text (brief first, then sections)."""
...
class TextStore:
    ...
    def __init__(
        ...
        self.chunk_to_file = {
            name: self.text_dir / f"{name}.json" for name in self.chunk_names
        }
        if self.verbose:
            print(
                f"[TextStore] using {len(self.chunk_names)}/"
                f"{len(all_chunk_names)} text chunks."
            )
    ...
    def get_texts(self, wids: List[str]) -> List[str]:
        return [self.get_text(w) for w in wids]
