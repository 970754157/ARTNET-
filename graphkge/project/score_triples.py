...
def _score_one_graph(bundle, head_idx: int, relation_idx: int, tail_idx: int) -> Dict:
    with torch.no_grad():
        ...
        relation = torch.tensor([relation_idx], dtype=torch.long, device=bundle.device)
        score = bundle.model.score_triples_from_embeddings(
    ...
    return {
        "head": int(head_idx),
        "relation": bundle.triple_store.idx_to_relation[int(relation_idx)],
...
def _score_batch_graph(bundle, head_idx: np.ndarray, relation_idx: np.ndarray, tail_idx: np.ndarray) -> List[Dict]:
    rows: List[Dict] = []
    for row_idx in range(int(head_idx.shape[0])):
        rows.append(
            _score_one_graph(
                ...
                relation_idx=int(relation_idx[row_idx]),
...
def main() -> None:
    ...
    if args.input:
        ...
        if bool(bundle.cfg.model.use_graph):
            rows = score_batch(bundle, head_idx=head_idx, relation_idx=relation_idx, tail_idx=tail_idx)
        else:
            rows = score_batch(
                bundle,
                head_idx=head_idx,
