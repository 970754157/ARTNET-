...
def _score_batch_nograph(bundle, head_idx: np.ndarray, tail_idx: np.ndarray, top_k: int, entity_batch_size: int) -> List[Dict]:
...
def _score_one_graph(bundle, head_idx: int, tail_idx: int, top_k: int) -> Dict:
    with torch.no_grad():
        prepared = build_prepared_graph_batch(
            bundle=bundle,
            seed_entities=np.asarray([head_idx, tail_idx], dtype=np.int64),
            masked_entity_pairs_global=np.asarray([[head_idx], [tail_idx]], dtype=np.int64),
        )
        emb = bundle.model.encode_subgraph(
        ...
        head_row = prepared.global_to_local[int(head_idx)]
        tail_row = prepared.global_to_local[int(tail_idx)]
        scores = bundle.model.score_relations_from_embeddings(
            emb[head_row : head_row + 1],
            emb[tail_row : tail_row + 1],
    ...
    return {
        ...
        "top_relations": [
            {
                "relation": bundle.triple_store.idx_to_relation[int(rel_idx)],
                "relation_idx": int(rel_idx),
                "score": float(scores[int(rel_idx)]),
            }
...
def _score_batch_graph(bundle, head_idx: np.ndarray, tail_idx: np.ndarray, top_k: int) -> List[Dict]:
    rows: List[Dict] = []
    for row_idx in range(int(head_idx.shape[0])):
        rows.append(
            _score_one_graph(
                bundle,
                head_idx=int(head_idx[row_idx]),
                tail_idx=int(tail_idx[row_idx]),
                top_k=top_k,
            )
        )
    return rows
...
def main() -> None:
    ...
    bundle.model.eval()
    score_batch = _score_batch_graph if bool(bundle.cfg.model.use_graph) else _score_batch_nograph
    if args.input:
        ...
        if bool(bundle.cfg.model.use_graph):
            rows = score_batch(bundle, head_idx=head_idx, tail_idx=tail_idx, top_k=args.top_k)
        else:
            rows = score_batch(
                bundle,
                head_idx=head_idx,
