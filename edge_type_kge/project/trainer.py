"""Training and evaluation for multimodal KGE."""
...
from project.experiment import ExperimentBundle, require_cuda_device
...
class Trainer:
    def __init__(self, bundle: ExperimentBundle, run_dir: Path):
        ...
        require_cuda_device(self.device, stage="trainer initialization")
    ...
    def _build_eval_entity_bank(self, entity_ids: np.ndarray) -> Tuple[np.ndarray, torch.Tensor]:
        ...
        dtype = next(self.model.parameters()).dtype
        bank_kwargs = {}
        if bool(self.cfg.runtime.pin_memory and self.device.type == "cuda"):
        ...
        with torch.no_grad():
            for start in range(0, int(entity_ids.shape[0]), step):
                ...
                chunk_emb = self.model.encode_entities(
                    ...
                    device=self.device,
                    pin_memory=bool(self.cfg.runtime.pin_memory and self.device.type == "cuda"),
                )
    ...
    def _evaluate_triple_scoring_prepared(
        self,
        pos_triples: np.ndarray,
    ...
    def _evaluate_relation_ranking_prepared(
        self,
        triples: np.ndarray,
        subset_mode: str,
        phase: str,
    ...
    def _run_eval_phase(self, split_idx: np.ndarray, max_triples: int, phase: str) -> Dict[str, float]:
        ...
        return {
            **self._evaluate_relation_ranking_prepared(
                pos_by_mode["global"],
                subset_mode="global",
                phase=phase,
            ...
            **self._evaluate_triple_scoring_prepared(
                pos_by_mode["global"],
                neg_by_mode["global"],
                subset_mode="global",
                phase=phase,
    ...
    def _batch_entity_embeddings(self, pos: np.ndarray, neg: np.ndarray) -> Tuple[np.ndarray, torch.Tensor]:
    ...
    def _score_triple_batch(self, pos: np.ndarray, neg: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
        unique_entities, entity_emb = self._batch_entity_embeddings(pos, neg)
    ...
    def _evaluate_triple_scoring(
        self,
        split_idx: np.ndarray,
        max_triples: int,
        subset_mode: str = "global",
        phase: str = "eval",
    ) -> Dict[str, float]:
        candidate_idx = self._subset_indices(split_idx, subset_mode)
        subset = self._sample_eval_subset(candidate_idx, max_triples)
        if subset.size == 0:
            return self._empty_triple_metrics()
        triples = self.bundle.triple_store.triples[subset]
        total_triples = int(triples.shape[0])
        total_batches = max(1, math.ceil(total_triples / max(1, int(self.cfg.data.eval_batch_size))))
        start_time = self._log_eval_task_start(phase=phase, task="triple_scoring", subset_mode=subset_mode, total_triples=total_triples)
        last_log_time = start_time
        ...
        self.model.eval()
        with torch.no_grad():
            for start in range(0, triples.shape[0], self.cfg.data.eval_batch_size):
                pos = triples[start : start + self.cfg.data.eval_batch_size]
                ...
                y_score.extend(pos_scores.detach().float().cpu().tolist())
                y_score.extend(neg_scores.detach().float().cpu().tolist())
                done_batches = min(total_batches, start // self.cfg.data.eval_batch_size + 1)
                done_triples = min(total_triples, start + pos.shape[0])
                last_log_time = self._maybe_log_eval_progress(
                    phase=phase,
                    task="triple_scoring",
                    subset_mode=subset_mode,
    ...
    def _evaluate_relation_ranking(
        self,
        split_idx: np.ndarray,
        max_triples: int,
        subset_mode: str = "global",
        ...
        with torch.no_grad():
            for start in range(0, triples.shape[0], self.cfg.data.eval_batch_size):
                batch = triples[start : start + self.cfg.data.eval_batch_size]
                unique_entities = np.unique(np.concatenate([batch[:, 0], batch[:, 2]], axis=0))
                entity_emb = self.model.encode_entities(
                    entity_indices=unique_entities,
                    catalog=self.bundle.catalog,
                    text_store=self.bundle.text_store,
                    image_provider=self.bundle.image_provider,
                ...
                head_rows = torch.from_numpy(self._lookup_rows(unique_entities, batch[:, 0])).to(self.device, dtype=torch.long)
                tail_rows = torch.from_numpy(self._lookup_rows(unique_entities, batch[:, 2])).to(self.device, dtype=torch.long)
                ...
                top1_hits.extend((np.argmax(score_np, axis=1) == batch[:, 1]).astype(np.float64).tolist())
                done_batches = min(total_batches, start // self.cfg.data.eval_batch_size + 1)
                done_triples = min(total_triples, start + batch.shape[0])
                last_log_time = self._maybe_log_eval_progress(
                    phase=phase,
                    task="relation_ranking",
                    subset_mode=subset_mode,
