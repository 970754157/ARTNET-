...
def build_experiment_bundle(
    ...
    log_event(
        ...
        (
            ...
            f"image_model={image_model_name} "
            f"image_model_dir={image_model_dir} kge_scorer={cfg.model.kge_scorer}"
        ),
    )
    ...
    if for_training:
        require_cuda_device(device, stage="training bundle build")
...
def prebuild_raw_feature_caches(bundle: ExperimentBundle) -> Dict[str, float]:
    require_cuda_device(bundle.device, stage="raw feature prebuild")
